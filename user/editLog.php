<?php

require_once("../config/database.php");

//$com="replace into student_log ( sid,type,reason,detail,logdate,addtime ) values(".$_POST["sid"].",'".$_POST["type"]."','".$_POST["reason"]."','".$_POST["detail"]."','".$_POST["logdate"]."','". date('Y-m-d H:i:s')."' )" ;

$com="update student_log set sid='".$_POST["sid"]."',reason='".$_POST["reason"]."',detail='".$_POST["detail"]."',addtime='". date('Y-m-d H:i:s')."' where sid='".$_POST["sid"]."' and addtime='".$_POST["addtime"]."'" ;

$result=mysqli_query($db,$com);
if($result){
    echo '<h4 style="margin:30px;">Tips:Information has been updated successfully!</h4>';
}
else{
    echo '<h4 style="margin:30px;">Attention:Information has been updated unsuccessfully！</h4>';
}

mysqli_close($db);
?>
<div style="width: 90%;height: 55px;margin: 50px"><div style="margin: 0 auto;width: 90px;height: 30px;background-color: #117700"><a style="text-decoration: none;padding:3px;color: #f3f3f3;text-align: center;display: block" href="./myLog.php">return</a></div> </div>
